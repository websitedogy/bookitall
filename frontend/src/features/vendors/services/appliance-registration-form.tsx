...
export function ApplianceRegistrationForm() {
  const router = useRouter();
  const user = useAuth((s) => s.user);
  const token = useAuth((s) => s.accessToken);

  const [name, setName] = useDraftState("appliance", "name", "");
  const [mobile, setMobile] = useDraftState("appliance", "mobile", "");
  const [experience, setExperience] = useDraftState("appliance", "experience", "");
  const [appliances, setAppliances] = useDraftState("appliance", "appliances", [] as string[]);
  const [services, setServices] = useDraftState("appliance", "services", [] as string[]);
  const [brands, setBrands] = useDraftState("appliance", "brands", [] as string[]);
  const [serviceMode, setServiceMode] = useDraftState<ServiceMode>("appliance", "serviceMode", "Home Service");
  const [tools, setTools] = useDraftState("appliance", "tools", [] as string[]);
  const [visitCharge, setVisitCharge] = useDraftState("appliance", "visitCharge", "");
  const [hourlyCharge, setHourlyCharge] = useDraftState("appliance", "hourlyCharge", "");
  const [fullDayCharge, setFullDayCharge] = useDraftState("appliance", "fullDayCharge", "");
  const [teamSize, setTeamSize] = useDraftState("appliance", "teamSize", "1 Person");
  const [spareParts, setSpareParts] = useDraftState("appliance", "spareParts", "Customer Provides");
  const [emergency, setEmergency] = useDraftState("appliance", "emergency", "Yes");
  const [days, setDays] = useDraftState("appliance", "days", [] as string[]);
  const [availableTime, setAvailableTime] = useDraftState("appliance", "availableTime", "Morning");
  const [locationLabel, setLocationLabel] = useDraftState("appliance", "locationLabel", "");
  const [lat, setLat] = useDraftState<number | null>("appliance", "lat", null);
  const [lng, setLng] = useDraftState<number | null>("appliance", "lng", null);
  const [district, setDistrict] = useDraftState("appliance", "district", "");
  const [coverage, setCoverage] = useDraftState<CoverageType>("appliance", "coverage", "Entire District");
  const [mandals, setMandals] = useDraftState("appliance", "mandals", [] as string[]);
  const [workPhotos, setWorkPhotos] = useDraftFiles("appliance", "workPhotos");
  ...
  if (!user || !token) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        <h1 className="text-3xl font-semibold tracking-tight">Sign in to list appliance services</h1>
        <Link href="/login?next=/vendors/services/appliance" className="mt-8 inline-flex h-12 items-center justify-center rounded-full bg-[var(--primary)] px-6 text-sm font-semibold text-white">
          Sign in to continue
        </Link>
      </div>
    );
  ...
  function validate() {
    if (!name.trim()) return "Enter business / technician name.";
    if (!/^\d{10}$/.test(mobile.replace(/\s/g, ""))) return "Enter a 10-digit mobile number.";
    if (!experience) return "Select experience.";
    if (!appliances.length) return "Select at least one appliance.";
    if (!services.length) return "Select at least one service.";
  ...
  async function submit() {
    ...
    try {
      ...
      body.append("technicianName", name.trim());
      ...
      body.append("applianceType", appliances.join(", "));
      body.append("servicesOffered", services.join(", "));
      body.append("serviceType", services.join(", "));
      body.append("brand", brands.join(", "));
      body.append("serviceMode", serviceMode);
      body.append("tools", tools.join(", "));
      body.append("spareParts", spareParts);
      body.append("material", spareParts);
      ...
      const res = await authFetch("/vendor-listings/appliance", { method: "POST", token, body });
      ...
      if (err instanceof ApiError && err.status === 401) {
        router.push("/login?next=/vendors/services/appliance");
        return;
      }
      ...
      <div className="md:mx-auto md:max-w-2xl md:px-0 md:pt-6 md:pb-8">
        <article className="bg-white md:rounded-2xl md:p-8 md:shadow-sm md:ring-1 md:ring-slate-200">
          <header className="sticky top-0 z-20 flex h-12 items-center gap-2.5 border-b border-slate-100 bg-white pl-12 pr-4 md:static md:h-auto md:border-0 md:px-0 md:pb-4">
            ...
            <h1 className="text-lg font-semibold tracking-tight text-slate-900 md:text-xl">Appliance</h1>
          </header>

          <div className="px-4 pb-6 md:px-0">
          <Section n={1} title="Basic Details">
            <Field label="Business / Technician Name" required>
          ...
          <Section n={2} title="Appliances Serviced" required>
            <CheckGrid values={APPLIANCES_SERVICED} selected={appliances} onToggle={(value) => toggle(appliances, value, setAppliances)} />
          </Section>

          <Section n={3} title="Services Offered" required>
            <CheckGrid values={APPLIANCE_SERVICES} selected={services} onToggle={(value) => toggle(services, value, setServices)} />
          </Section>

          <Section n={4} title="Brands & Service Mode">
            <p className="text-sm font-medium text-slate-800">Brands Supported</p>
            <CheckGrid values={APPLIANCE_BRANDS} selected={brands} onToggle={(value) => toggle(brands, value, setBrands)} />
              ...
              {SERVICE_MODES.map((option) => (
                <button
                  ...
                  className={cn(
                    ...
                    serviceMode === option ? "border-blue-500 text-blue-700" : "border-[#d7dde6] text-slate-700",
                  )}
                >
                  <span className={cn("inline-flex h-4 w-4 items-center justify-center rounded-full border", serviceMode === option ? "border-blue-500" : "border-slate-400")}>
                    {serviceMode === option ? <span className="h-2 w-2 rounded-full bg-blue-500" /> : null}
                  </span>
                  {option}
                </button>
              ))}
          ...
          <Section n={5} title="Charges, Parts & Team">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Spare Parts">
                <select value={spareParts} onChange={(e) => setSpareParts(e.target.value)} className={inputClass}>
                  {SPARE_PARTS_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {option}
                    </option>
                  ))}
              ...
              <Field label="Emergency / Same-Day">
            ...
            <CheckGrid values={APPLIANCE_TOOLS} selected={tools} onToggle={(value) => toggle(tools, value, setTools)} />

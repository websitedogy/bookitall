...
import { GOODS_TRANSPORT_TYPES, goodsTransportTypeHref } from "./goods-transport-data";
import { GoodsTransportIcon } from "./goods-transport-icons";
import { cn } from "@/shared/lib/cn";

export function GoodsTransportTypePicker({
  baseHref,
  selected,
  compact = false,
  title = "Goods Transport",
  subtitle = "Choose a vehicle type, then complete the same registration form.",
  ...
  return (
    <section className={cn(compact ? "px-3 pt-3 md:px-7" : "px-4 py-6 md:px-0")}>
      ...
      <ul className={cn("grid grid-cols-3 gap-2 md:grid-cols-6", compact ? "md:gap-3" : "md:gap-4")}>
        {GOODS_TRANSPORT_TYPES.map((item) => {
          const active = selected === item.id;
          return (
            <li key={item.id}>
              <Link
                href={goodsTransportTypeHref(baseHref, item.id)}
                prefetch
                className={cn(

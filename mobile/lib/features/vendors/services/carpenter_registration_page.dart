...
import 'carpenter_form_data.dart';
import 'electrician_form_data.dart';
import 'listing_draft.dart';
import 'vendor_location_picker.dart';

class CarpenterRegistrationPage extends StatefulWidget {
  const CarpenterRegistrationPage({super.key});

  @override
  State<CarpenterRegistrationPage> createState() => _CarpenterRegistrationPageState();
}

class _CarpenterRegistrationPageState extends State<CarpenterRegistrationPage> {
  ...
  String? _validate() {
    if (_name.text.trim().isEmpty) return 'Enter business / carpenter name.';
    if (!RegExp(r'^\d{10}$').hasMatch(_mobile.text.replaceAll(RegExp(r'\s'), ''))) {
      return 'Enter a 10-digit mobile number.';
    }
    if (_experience.isEmpty) return 'Select experience.';
    if (_services.isEmpty) return 'Select at least one woodwork service.';
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final fields = <String, String>{
        ...
        'serviceType': _customerTypes.isEmpty ? _workTypes.first : _customerTypes.first,
      ...
      await submitVendorListing(category: 'carpenter', fields: fields, photoPaths: photos);
      PostsRefresh.bump();
      if (!mounted) return;
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                _section(1, 'Details'),
                _label('Business / Carpenter Name', required: true),
                ...
                _checks(carpenterServices, _services),
                _section(3, 'Work type', required: true),
                _label('Work Type', required: true),
                _checks(carpenterWorkTypes, _workTypes),
                const SizedBox(height: 10),
                _label('Customer Type'),
                _checks(customerTypes, _customerTypes),
                _section(4, 'Tools'),
                _checks(carpenterTools, _tools),
                _section(5, 'Charges'),
                Row(
                  children: [
                    Expanded(

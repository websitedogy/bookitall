...
class TradeRegistrationPage extends StatefulWidget {
  const TradeRegistrationPage({super.key, required this.kind});

  final String kind;

  @override
  State<TradeRegistrationPage> createState() => _TradeRegistrationPageState();
}

class _TradeRegistrationPageState extends State<TradeRegistrationPage> {
  final _name = TextEditingController();
  final _mobile = TextEditingController();
  final _location = TextEditingController();
  final _visitAmount = TextEditingController();
  final _hourlyAmount = TextEditingController();
  final _fullDayAmount = TextEditingController();
  ...
  String _experience = '';
  String _visitCharge = '';
  String _hourlyCharge = '';
  String _fullDayCharge = 'Not Available';
  ...
  final _customerTypes = <String>{};
  final _acTypes = <String>{};
  final _acBrands = <String>{};
  final _days = <String>{};
  ...
  void dispose() {
    ...
    _mobile.dispose();
    _location.dispose();
    _visitAmount.dispose();
    _hourlyAmount.dispose();
    _fullDayAmount.dispose();
    _extraInfo.dispose();
    _serviceKm.dispose();
    super.dispose();
  }

  bool get _isElectrician => widget.kind == 'electrician';
  bool get _isPlumber => widget.kind == 'plumber';
  bool get _mapLocation => _isElectrician || _isPlumber || _isAc;
  bool get _amountCharges => _isElectrician || _isPlumber || _isAc;
  bool get _hideProfilePhoto => _isElectrician || _isPlumber || _isAc;
  bool get _hideHourlyFullDay => false;
  bool get _showAdditionalInfo => _isElectrician || _isPlumber || _isAc;
  bool get _isCleaning => widget.kind == 'cleaning';
  bool get _isAc => widget.kind == 'ac';
  bool get _extraCharges => _isPlumber || _isCleaning || _isAc;
  String get _title => _isAc
      ? 'AC'
      : _isCleaning
          ? 'Cleaning'
          : _isPlumber
              ? 'Plumber'
              : 'Electrician';
  String get _nameLabel => _isAc
      ? 'Business / Technician Name'
      : _isCleaning
          ? 'Business / Cleaning Name'
          : _isPlumber
              ? 'Plumber / Business Name'
              : 'Business / Electrician Name';
  String get _nameField => _isAc
      ? 'technicianName'
      : _isCleaning
          ? 'cleanerName'
          : _isPlumber
              ? 'plumberName'
              : 'electricianName';
  String get _servicesTitle => 'Services';
  String get _audienceTitle => _isAc
      ? 'AC types'
      : _isCleaning
          ? 'Property type'
          : 'Customers';
  String get _chargesTitle => 'Charges';
  String get _chargeLabel => _isCleaning ? 'Starting Service Charge' : 'Visit Charge';
  String get _emergencyLabel => _isAc || _isCleaning ? 'Emergency / Same-Day Service' : 'Emergency Service';
  List<String> get _serviceOptions => _isAc
      ? acServices
      : _isCleaning
          ? cleaningServices
          : _isPlumber
              ? plumberServices
              : electricianServices;
  List<String> get _audienceOptions => _isCleaning ? propertyTypes : customerTypes;
  List<String> get _chargeOptions => _isCleaning ? startingCharges : visitCharges;
  ...
  String? _validate() {
    if (_name.text.trim().isEmpty) return 'Enter ${_nameLabel.toLowerCase()}.';
    ...
    if (_isAc) {
      if (_acTypes.isEmpty) return 'Select AC types.';
    } else if (_customerTypes.isEmpty) {
      return 'Select ${_audienceTitle.toLowerCase()}.';
    }
    if ((_amountCharges ? _visitAmount.text : _visitCharge).isEmpty) {
      return _amountCharges ? 'Enter ${_chargeLabel.toLowerCase()}.' : 'Select ${_chargeLabel.toLowerCase()}.';
    }
    if (_extraCharges && !_hideHourlyFullDay && (_amountCharges ? _hourlyAmount.text : _hourlyCharge).isEmpty) {
      return _amountCharges ? 'Enter hourly charge.' : 'Select hourly charge.';
    }
    if (_location.text.trim().isEmpty) return 'Detect or enter your current location.';
    if (_mapLocation && _serviceKm.text.trim().isEmpty) return 'Enter service area in KM.';
    if (!_mapLocation && _district.isEmpty) return 'Select district.';
    if (!_mapLocation && _coverage == 'Selected Mandals' && _mandals.isEmpty) return 'Select at least one mandal.';
    return null;
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final fields = <String, String>{
        ...
        'workType': _services.first,
        'cleaningType': _services.first,
        'customerType': _customerTypes.join(', '),
        'serviceType': _isAc ? _acTypes.first : _customerTypes.first,
        'propertyType': _customerTypes.join(', '),
        if (_isAc) 'acType': _acTypes.join(', '),
        if (_isAc) 'brand': _acBrands.join(', '),
        'visitCharge': _amountCharges ? _visitAmount.text.trim() : _visitCharge,
        'startingCharge': _amountCharges ? _visitAmount.text.trim() : _visitCharge,
        'price': '${visitChargeAmount(_amountCharges ? _visitAmount.text : _visitCharge)}',
        'priceUnit': 'PER_VISIT',
        if (_extraCharges && !_hideHourlyFullDay) 'hourlyCharge': _amountCharges ? _hourlyAmount.text.trim() : _hourlyCharge,
        if (_extraCharges && !_hideHourlyFullDay) 'fullDayCharge': _amountCharges ? _fullDayAmount.text.trim() : _fullDayCharge,
        if (_extraCharges) 'teamSize': _teamSize,
        'emergency': _emergency,
        'availableDays': _days.join(', '),
        'availableTime': _availableTime,
        'location': _location.text.trim(),
        if (_mapLocation) 'serviceKm': _serviceKm.text.trim(),
        if (_mapLocation) 'serviceArea': 'Within ${_serviceKm.text.trim()} KM',
        if (_mapLocation) 'coverage': 'Within ${_serviceKm.text.trim()} KM',
        'district': _district,
        'state': info?.state ?? '',
        'city': _district,
        'area': _mapLocation ? _location.text.trim() : (_coverage == 'Selected Mandals' ? _mandals.join(', ') : _district),
        if (!_mapLocation) 'coverageType': _coverage,
        if (!_mapLocation) 'coverage': _coverage == 'Entire District' ? 'Entire $_district' : _mandals.join(', '),
        if (!_mapLocation) 'mandals': _mandals.join(', '),
        if (_lat != null) 'latitude': _lat.toString(),
        if (_lng != null) 'longitude': _lng.toString(),
        if (_showAdditionalInfo && _extraInfo.text.trim().isNotEmpty) 'description': _extraInfo.text.trim(),
      };
      final photos = <String>[
        if (!_hideProfilePhoto && _profile != null) _profile!.path,
        ..._workPhotos.map((file) => file.path),
      ].take(10).toList();
      await submitVendorListing(category: widget.kind, fields: fields, photoPaths: photos);
      PostsRefresh.bump();
      if (!mounted) return;
  ...
  Future<void> _pickProfile() async {
    ...
    if (file != null) setState(() => _profile = file);
  }
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                _section(1, 'Details'),
                _label(_nameLabel, required: true),
                ...
                if (!_hideProfilePhoto) ...[
                  const SizedBox(height: 14),
                  _label('Profile / Business Photo'),
                  const SizedBox(height: 6),
                  OutlinedButton(
                    onPressed: _pickProfile,
                    child: Text(_profile == null ? 'Choose File' : _profile!.name),
                  ),
                ],
                _section(2, _servicesTitle, required: true),
                _checks(_serviceOptions, _services),
                _section(3, _audienceTitle, required: _isAc),
                if (_isAc) ...[
                  _label('AC Types', required: true),
                  _checks(acTypes, _acTypes),
                  const SizedBox(height: 10),
                  _label('Brands Supported'),
                  _checks(acBrands, _acBrands),
                ] else
                  _checks(_audienceOptions, _customerTypes),
                _section(4, _chargesTitle),
                Row(
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _label(_chargeLabel, required: true),
                          if (_amountCharges)
                            _field(_visitAmount, hint: '₹ Enter amount', keyboard: TextInputType.number, digits: 6)
                          else
                            _dropdown(_visitCharge, 'Select', _chargeOptions, (value) => setState(() => _visitCharge = value)),
                        ],
                      ),
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          if (_extraCharges && !_hideHourlyFullDay) ...[
                            _label('Hourly Charge', required: true),
                            if (_amountCharges)
                              _field(_hourlyAmount, hint: '₹ Enter amount', keyboard: TextInputType.number, digits: 6)
                            else
                              _dropdown(_hourlyCharge, 'Select', hourlyCharges, (value) => setState(() => _hourlyCharge = value)),
                          ] else ...[
                            _label(_emergencyLabel),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_amountCharges)
                                _field(_fullDayAmount, hint: '₹ Enter amount', keyboard: TextInputType.number, digits: 6)
                              else
                                _dropdown(_fullDayCharge, 'Not Available', fullDayCharges, (value) => setState(() => _fullDayCharge = value)),
                            ],
                          ),
                        ),
                        const SizedBox(width: 12),
                ...
                if (_showAdditionalInfo) ...[
                  _section(7, 'Additional Information'),
                  TextField(

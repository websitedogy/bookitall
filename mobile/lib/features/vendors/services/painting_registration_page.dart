...
import 'painting_form_data.dart';
import 'vendor_location_picker.dart';

class PaintingRegistrationPage extends StatefulWidget {
  const PaintingRegistrationPage({super.key});

  @override
  State<PaintingRegistrationPage> createState() => _PaintingRegistrationPageState();
}

class _PaintingRegistrationPageState extends State<PaintingRegistrationPage> {
  ...
  final _workTypes = <String>{};
  final _propertyTypes = <String>{};
  final _paintKinds = <String>{};
  final _tools = <String>{};
  final _days = <String>{};
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final fields = <String, String>{
        ...
        'paintingType': _services.join(', '),
        'workType': _workTypes.join(', '),
        'serviceType': _workTypes.first,
        'propertyType': _propertyTypes.join(', '),
        'paintTypes': _paintKinds.join(', '),
        'tools': _tools.join(', '),
        'materialResponsibility': _material,
        'material': _material,
      ...
      await submitVendorListing(category: 'painting', fields: fields, photoPaths: photos);
      PostsRefresh.bump();
      if (!mounted) return;
        ...
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                _section(1, 'Details'),
                _label('Business / Painter Name', required: true),
                ...
                _section(2, 'Services', required: true),
                _checks(paintingServices, _services),
                _section(3, 'Work type', required: true),
                _label('Property Type'),
                _checks(paintingPropertyTypes, _propertyTypes),
                const SizedBox(height: 10),
                _label('Work Type', required: true),
                _checks(paintingWorkTypes, _workTypes),
                _section(4, 'Materials'),
                _label('Paint Types Supported'),
                _checks(paintTypes, _paintKinds),
                const SizedBox(height: 14),
                _label('Material Responsibility', required: true),
                _dropdown(_material, 'Select', materialOptions, (value) => setState(() => _material = value)),
                const SizedBox(height: 14),
                _label('Service Team Size'),
                _dropdown(_teamSize, '1 Person', teamSizes, (value) => setState(() => _teamSize = value)),
                const SizedBox(height: 14),
                _label('Tools Available'),
                _checks(paintingTools, _tools),
                _section(5, 'Charges'),
                Row(
                  children: [
                    Expanded(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          _label('Emergency / Same-Day Service'),
